...
<div class="container-fluid page-header py-5">
	<h1 class="text-center text-white display-6">포트폴리오</h1>
...
<div class="portfolio-dashboard-wrap">
	...
	<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 18px;">
		<c:if test="${empty portfolios}">
			<form method="post" action="<c:url value='/portfolio/createResumePortfolio.do'/>" style="margin: 0;">
				...
				<c:choose>
					<c:when test="${empty resume}">
						<div style="padding: 28px 22px;">
							<h3 style="margin: 0 0 10px 0; font-size: 1.4rem; font-weight: 800;">저장된 레쥬메가 없어요</h3>
							<p style="margin: 0; color: #666; line-height: 1.6;">먼저 스냅샷 데이터가 있어야 왼쪽에 레쥬메 정보가 표시돼요.</p>
						</div>
					</c:when>
						...
						<div class="resume-preview-wrap">
							<div class="resume-preview-grid">
								...
								<div class="resume-left-col">
									...
									<h1 class="resume-name view-mode">${resume.name}</h1>
									<input type="text" class="form-control edit-mode"
										id="resume-name" value="${resume.name}"
										style="display:none; margin-bottom:10px;" />

									<c:choose>
										<c:when test="${not empty resume.headline}">
											<div class="resume-headline view-mode">${resume.headline}</div>
										</c:when>
										<c:otherwise>
									...
									<input type="text" class="form-control edit-mode"
										id="resume-headline" value="${resume.headline}"
									...
									<div class="resume-block">
										...
										<div class="view-mode">
											<c:choose>
												<c:when test="${not empty resume.email or not empty resume.phone or not empty resume.location}">
										...
										<div class="edit-mode" style="display:none;">
											<input type="text" class="form-control mb-2" id="resume-email" value="${resume.email}" placeholder="이메일" />
											<input type="text" class="form-control mb-2" id="resume-phone" value="${resume.phone}" placeholder="전화번호" />
											<input type="text" class="form-control" id="resume-location" value="${resume.location}" placeholder="위치" />
										</div>
									</div>

									<div class="resume-block">
										<div class="resume-label">Links</div>

										<div class="view-mode">
											<c:choose>
												<c:when test="${not empty resume.socialLinks}">
													<c:forEach var="l" items="${resume.socialLinks}">
														<div class="resume-link-item">${l.label} : ${l.url}</div>
													</c:forEach>
												</c:when>
												<c:otherwise>
													<div class="resume-empty">등록된 링크가 없습니다.</div>
												</c:otherwise>
											</c:choose>
										</div>

										<div class="edit-mode" id="links-editor" style="display:none;">
											<c:choose>
												<c:when test="${not empty resume.socialLinks}">
													<c:forEach var="l" items="${resume.socialLinks}">
														<div class="edit-section-box link-row">
															<input type="text" class="form-control mb-2 link-label" value="${l.label}" placeholder="라벨" />
															<input type="text" class="form-control mb-2 link-url" value="${l.url}" placeholder="URL" />
												...
												<c:when test="${not empty resume.skills}">
													<div class="resume-badges">
														<c:forEach var="s" items="${resume.skills}">
															<span class="resume-badge">${s}</span>
														</c:forEach>
													</div>
												</c:when>
												<c:otherwise>
													<div class="resume-empty">등록된 스킬이 없습니다.</div>
												</c:otherwise>
											</c:choose>
										</div>

										<div class="edit-mode" style="display:none;">
											<textarea class="form-control" id="resume-skills" rows="4" placeholder="스킬을 줄바꿈 또는 쉼표로 구분해서 입력"><c:if test="${not empty resume.skills}"><c:forEach var="s" items="${resume.skills}" varStatus="st">${s}<c:if test="${!st.last}">
									...
									<div class="resume-block">
										...
										<div class="view-mode">
											<c:choose>
												<c:when test="${not empty resume.summary}">
													<div class="resume-summary">${resume.summary}</div>
												</c:when>
												<c:otherwise>
													<div class="resume-empty">등록된 자기소개가 없습니다.</div>
												</c:otherwise>
											</c:choose>
										</div>

										<textarea class="form-control edit-mode" id="resume-summary"
											rows="6" style="display:none;"
											placeholder="자기소개">${resume.summary}</textarea>
									</div>
								</div>
								...
								<div class="resume-right-col">
									...
									<div class="resume-block">
													...
													<c:forEach var="p" items="${resume.projects}">
														<div class="resume-item">
															<div class="resume-item-top">
																...
																<div class="resume-item-meta">${p.period}</div>
															</div>

															<c:if test="${not empty p.role}">
																<div class="resume-item-sub">${p.role}</div>
															</c:if>

															<c:if test="${not empty p.oneLine}">
																<div class="resume-item-desc">${p.oneLine}</div>
															</c:if>
										...
										<div class="edit-mode" id="projects-editor" style="display:none;">
											<c:choose>
												<c:when test="${not empty resume.projects}">
													<c:forEach var="p" items="${resume.projects}">
														<div class="edit-section-box project-row">
															<input type="text" class="form-control mb-2 project-title" value="${p.title}" placeholder="프로젝트명" />
															<input type="text" class="form-control mb-2 project-period" value="${p.period}" placeholder="기간" />
															<input type="text" class="form-control mb-2 project-role" value="${p.role}" placeholder="역할" />
															<textarea class="form-control mb-2 project-oneline" rows="2" placeholder="한 줄 소개">${p.oneLine}</textarea>
															<textarea class="form-control mb-2 project-highlights" rows="4" placeholder="하이라이트(줄바꿈으로 구분)"><c:if test="${not empty p.highlights}"><c:forEach var="h" items="${p.highlights}" varStatus="hst">${h}<c:if test="${!hst.last}">
</c:if></c:forEach></c:if></textarea>
															<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
														</div>
													</c:forEach>
									...
									<div class="resume-block">
											...
											<c:choose>
												<c:when test="${not empty resume.experience}">
													<c:forEach var="e" items="${resume.experience}">
														<div class="resume-item">
															<div class="resume-item-top">
																<div class="resume-item-title">${e.title}</div>
																<div class="resume-item-meta">${e.period}</div>
															</div>

															<c:if test="${not empty e.company}">
																<div class="resume-item-sub">${e.company}</div>
															</c:if>
										...
										<div class="edit-mode" id="experience-editor" style="display:none;">
											<c:choose>
												<c:when test="${not empty resume.experience}">
													<c:forEach var="e" items="${resume.experience}">
														<div class="edit-section-box experience-row">
															<input type="text" class="form-control mb-2 exp-title" value="${e.title}" placeholder="직무명" />
															<input type="text" class="form-control mb-2 exp-company" value="${e.company}" placeholder="회사명" />
															<input type="text" class="form-control mb-2 exp-period" value="${e.period}" placeholder="기간" />
															<textarea class="form-control mb-2 exp-description" rows="4" placeholder="업무 내용(줄바꿈으로 구분)"><c:if test="${not empty e.description}"><c:forEach var="d" items="${e.description}" varStatus="dst">${d}<c:if test="${!dst.last}">
</c:if></c:forEach></c:if></textarea>
															<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
														</div>
													</c:forEach>
									...
									<div class="resume-bottom-grid">
										...
										<div class="resume-bottom-col">
											<div class="resume-block">
													...
													<c:choose>
														<c:when test="${not empty resume.education}">
															<c:forEach var="ed" items="${resume.education}">
																<div class="resume-item">
																	<div class="resume-item-top">
																		<div class="resume-item-title">${ed.degree}</div>
																	...
																	<c:if test="${not empty ed.school}">
																		<div class="resume-item-sub">${ed.school}</div>
																	</c:if>

																	<c:if test="${not empty ed.major}">
																		<div class="resume-item-desc">${ed.major}</div>
																	</c:if>
																</div>
															</c:forEach>
														</c:when>
												...
												<div class="edit-mode" id="education-editor" style="display:none;">
													<c:choose>
														<c:when test="${not empty resume.education}">
															<c:forEach var="ed" items="${resume.education}">
																<div class="edit-section-box education-row">
																	<input type="text" class="form-control mb-2 edu-degree" value="${ed.degree}" placeholder="학위" />
																	<input type="text" class="form-control mb-2 edu-school" value="${ed.school}" placeholder="학교명" />
																	<input type="text" class="form-control mb-2 edu-major" value="${ed.major}" placeholder="전공" />
																	<input type="text" class="form-control mb-2 edu-start" value="${ed.startDate}" placeholder="시작일" />
																	<input type="text" class="form-control mb-2 edu-end" value="${ed.endDate}" placeholder="종료일" />
																	<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
																</div>
															</c:forEach>
														</c:when>
										...
										<div class="resume-bottom-col">
											<div class="resume-block">
													...
													<c:choose>
														<c:when test="${not empty resume.certificationItems}">
															<c:forEach var="c" items="${resume.certificationItems}">
																<div class="resume-item">
																	<div class="resume-item-title">${c.certi_name}</div>
																	<c:if test="${not empty c.organizer}">
																		<div class="resume-item-sub">${c.organizer}</div>
																	</c:if>
																</div>
															</c:forEach>
														</c:when>
												...
												<div class="edit-mode" id="certifications-editor" style="display:none;">
													<c:choose>
														<c:when test="${not empty resume.certificationItems}">
															<c:forEach var="c" items="${resume.certificationItems}">
																<div class="edit-section-box cert-row">
																	<input type="text" class="form-control mb-2 cert-name" value="${c.certi_name}" placeholder="자격증명" />
																	<input type="text" class="form-control mb-2 cert-org" value="${c.organizer}" placeholder="주최기관" />
																	<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
																</div>
															</c:forEach>
														</c:when>
			...
			<div class="panel-card">
				...
				<div style="padding: 24px 20px;">
					...
					<p style="margin: 0 0 16px 0; color: #666; line-height: 1.6;">
						1장 레쥬메 포트폴리오를 먼저 생성하면, 여기서 미니멀/디자인 PDF를 만들 수 있어요.
					</p>
...
<script>
  ...
  function toggleResumeEdit(editMode) {
    ...
    if (cancelBtn) cancelBtn.style.display = editMode ? '' : 'none';
  }
  ...
  function addLinkRow() {
    ...
    `;
    document.getElementById('links-editor').insertAdjacentHTML('beforeend', html);
  }
  ...
  function addProjectRow() {
    ...
    `;
    document.getElementById('projects-editor').insertAdjacentHTML('beforeend', html);
  }
  ...
  function addExperienceRow() {
    ...
    `;
    document.getElementById('experience-editor').insertAdjacentHTML('beforeend', html);
  }
  ...
  function addEducationRow() {
    ...
    `;
    document.getElementById('education-editor').insertAdjacentHTML('beforeend', html);
  }
  ...
  function addCertificationRow() {
    ...
    `;
    document.getElementById('certifications-editor').insertAdjacentHTML('beforeend', html);
  }
  ...
  function collectResumeJson() {
    ...
    if (projectsEditor) {
      projectsEditor.querySelectorAll('.project-row').forEach(function(row) {
        ...
        if (title || period || role || oneLine || highlights.length > 0) {
          projects.push({ title, period, role, oneLine, highlights });
        }
      });
    }
    ...
    if (experienceEditor) {
      experienceEditor.querySelectorAll('.experience-row').forEach(function(row) {
    ...
    if (educationEditor) {
      educationEditor.querySelectorAll('.education-row').forEach(function(row) {
        ...
        if (degree || school || major || startDate || endDate) {
          education.push({ degree, school, major, startDate, endDate });
        }
      });
    }
    ...
    if (certEditor) {
      certEditor.querySelectorAll('.cert-row').forEach(function(row) {
        ...
        if (certi_name || organizer) {
          certificationItems.push({ certi_name, organizer });
        }
      });
    }
    ...
    if (linksEditor) {
      ...
      items.forEach(function(item) {
        linksEditor.insertAdjacentHTML('beforeend', `
          <div class="edit-section-box link-row">
            <input type="text" class="form-control mb-2 link-label" value="${escapeHtml(item.label || '')}" placeholder="라벨" />
            <input type="text" class="form-control mb-2 link-url" value="${escapeHtml(item.url || '')}" placeholder="URL" />
            <button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
          </div>
        `);
      });
      linksEditor.insertAdjacentHTML('beforeend', `<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addLinkRow()">링크 추가</button>`);
    }
    ...
    if (projectsEditor) {
      ...
      items.forEach(function(item) {
        projectsEditor.insertAdjacentHTML('beforeend', `
          <div class="edit-section-box project-row">
            <input type="text" class="form-control mb-2 project-title" value="${escapeHtml(item.title || '')}" placeholder="프로젝트명" />
            <input type="text" class="form-control mb-2 project-period" value="${escapeHtml(item.period || '')}" placeholder="기간" />
            <input type="text" class="form-control mb-2 project-role" value="${escapeHtml(item.role || '')}" placeholder="역할" />
            <textarea class="form-control mb-2 project-oneline" rows="2" placeholder="한 줄 소개">${escapeHtml(item.oneLine || '')}</textarea>
            <textarea class="form-control mb-2 project-highlights" rows="4" placeholder="하이라이트(줄바꿈으로 구분)">${escapeHtml(joinLines(item.highlights || []))}</textarea>
            <button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
          </div>
        `);
      });
      projectsEditor.insertAdjacentHTML('beforeend', `<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addProjectRow()">프로젝트 추가</button>`);
    }
    ...
    if (experienceEditor) {
      ...
      items.forEach(function(item) {
        experienceEditor.insertAdjacentHTML('beforeend', `
          <div class="edit-section-box experience-row">
            <input type="text" class="form-control mb-2 exp-title" value="${escapeHtml(item.title || '')}" placeholder="직무명" />
            <input type="text" class="form-control mb-2 exp-company" value="${escapeHtml(item.company || '')}" placeholder="회사명" />
            <input type="text" class="form-control mb-2 exp-period" value="${escapeHtml(item.period || '')}" placeholder="기간" />
            <textarea class="form-control mb-2 exp-description" rows="4" placeholder="업무 내용(줄바꿈으로 구분)">${escapeHtml(joinLines(item.description || []))}</textarea>
            <button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
          </div>
        `);
      });
      experienceEditor.insertAdjacentHTML('beforeend', `<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addExperienceRow()">경력 추가</button>`);
    }
    ...
    if (educationEditor) {
      ...
      items.forEach(function(item) {
        educationEditor.insertAdjacentHTML('beforeend', `
          <div class="edit-section-box education-row">
            <input type="text" class="form-control mb-2 edu-degree" value="${escapeHtml(item.degree || '')}" placeholder="학위" />
            <input type="text" class="form-control mb-2 edu-school" value="${escapeHtml(item.school || '')}" placeholder="학교명" />
            <input type="text" class="form-control mb-2 edu-major" value="${escapeHtml(item.major || '')}" placeholder="전공" />
            <input type="text" class="form-control mb-2 edu-start" value="${escapeHtml(item.startDate || '')}" placeholder="시작일" />
            <input type="text" class="form-control mb-2 edu-end" value="${escapeHtml(item.endDate || '')}" placeholder="종료일" />
            <button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
          </div>
        `);
      });
      educationEditor.insertAdjacentHTML('beforeend', `<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addEducationRow()">학력 추가</button>`);
    }
    ...
    if (certEditor) {
      ...
      items.forEach(function(item) {
        certEditor.insertAdjacentHTML('beforeend', `
          <div class="edit-section-box cert-row">
            <input type="text" class="form-control mb-2 cert-name" value="${escapeHtml(item.certi_name || '')}" placeholder="자격증명" />
            <input type="text" class="form-control mb-2 cert-org" value="${escapeHtml(item.organizer || '')}" placeholder="주최기관" />
            <button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>
          </div>
        `);
      });
      certEditor.insertAdjacentHTML('beforeend', `<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addCertificationRow()">자격증 추가</button>`);
    }
  }
  ...
  async function saveResumeSnapshot(portfolioId) {
    ...
    try {
      const response = await fetch('<c:url value="/portfolio/snapshot/update.do"/>', {
        method: 'POST',
        headers: {
