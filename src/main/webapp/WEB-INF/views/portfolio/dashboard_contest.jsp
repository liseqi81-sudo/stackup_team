...
<div class="container-fluid page-header py-5"
	...
	<ol class="breadcrumb justify-content-center mb-0">
		<li class="breadcrumb-item"><a href="<c:url value='/main.do'/>"
			style="color: #81c408 !important;">Home</a></li>
		<li class="breadcrumb-item active" style="color: #777 !important;">Portfolio</li>
	</ol>
</div>
						...
						<div class="resume-preview-wrap">
							<div class="resume-preview-grid">
								...
								<div class="resume-left-col">
									...
									<div class="resume-block">
										...
										<div class="edit-mode" style="display: none;">
											...
											<div class="small text-muted mt-2">연락처 정보는 이 화면에서 수정할 수
												없습니다.</div>
										</div>
									</div>
									<div class="resume-block">
										...
										<div class="edit-mode" id="links-editor"
											style="display: none;">
											<c:choose>
												<c:when test="${not empty resume.socialLinks}">
													<c:forEach var="l" items="${resume.socialLinks}">
														<div class="edit-section-box link-row">
															<input type="text" class="form-control mb-2 link-label"
																value="${fn:escapeXml(l.label)}" placeholder="라벨" /> <input
																type="text" class="form-control mb-2 link-url"
																value="${fn:escapeXml(l.url)}" placeholder="URL" />
															<button type="button"
												...
												<c:otherwise>
													<div class="edit-section-box link-row">
														<input type="text" class="form-control mb-2 link-label"
															placeholder="라벨" /> <input type="text"
															class="form-control mb-2 link-url" placeholder="URL" />
														<button type="button"
															class="btn btn-sm btn-outline-danger editor-remove-btn"
															onclick="removeEditorRow(this)">삭제</button>
													</div>
											...
											<button type="button"
												...
												onclick="addLinkRow()">링크 추가</button>
										</div>
									</div>
												...
												<c:when test="${not empty resume.skills}">
													<div class="resume-badges">
														<c:forEach var="s" items="${resume.skills}">
															<span class="resume-badge"><c:out value="${s}" /></span>
														</c:forEach>
													</div>
												</c:when>
												<c:otherwise>
										...
										<div class="edit-mode" style="display: none;">
											<c:choose>
												<c:when test="${not empty resume.skills}">
													<div class="resume-badges">
														<c:forEach var="s" items="${resume.skills}">
															<span class="resume-badge"> <c:out value="${s}" />
															</span>
														</c:forEach>
													</div>
												</c:when>
											...
											<div class="small text-muted mt-2">스킬 정보는 이 화면에서 수정할 수
												없습니다.</div>
										</div>
									</div>
									...
									<div class="resume-block">
										...
										<textarea class="form-control edit-mode" id="resume-summary"
											rows="6" style="display: none;" placeholder="자기소개">${fn:escapeXml(resume.summary)}</textarea>
									</div>
								</div>
								...
								<div class="resume-right-col">
												...
												<c:when test="${not empty resume.projects}">
													<c:forEach var="p" items="${resume.projects}">
														<div class="resume-item">
															...
															<div class="resume-item-line">
																<span class="resume-item-label-inline">프로젝트명 :</span> <span
																	class="resume-item-value-inline"><c:out
																		value="${p.title}" /></span>
															</div>

															<c:if test="${not empty p.period}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">기간 :</span> <span
																		class="resume-item-value-inline"><c:out
																			value="${p.period}" /></span>
																</div>
															</c:if>
															...
															<c:if test="${not empty p.role}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">역할 :</span> <span
																		class="resume-item-value-inline"><c:out
																			value="${p.role}" /></span>
																</div>
															</c:if>
															...
															<c:if test="${not empty p.oneLine}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">한 줄 소개 :</span>
																	<span class="resume-item-value-inline"><c:out
																			value="${p.oneLine}" /></span>
																</div>
															</c:if>

															<c:if test="${not empty p.highlights}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">프로젝트 소개 :</span>
																</div>
																<ul class="resume-item-bullets">
																	<c:forEach var="h" items="${p.highlights}">
																		<li><c:out value="${h}" /></li>
												...
												<c:when test="${not empty resume.experience}">
													<c:forEach var="e" items="${resume.experience}">
														<div class="resume-item">
															...
															<div class="resume-item-line">
																<span class="resume-item-label-inline">직무명 :</span> <span
																	class="resume-item-value-inline"><c:out
																		value="${e.title}" /></span>
															</div>
															...
															<c:if test="${not empty e.company}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">회사명 :</span> <span
																		class="resume-item-value-inline"><c:out
																			value="${e.company}" /></span>
																</div>
															</c:if>
															...
															<c:if test="${not empty e.description}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">업무 내용 :</span>
																</div>
																<ul class="resume-item-bullets">
																	<c:forEach var="d" items="${e.description}">
																		<li><c:out value="${d}" /></li>
														...
														<c:when test="${not empty resume.education}">
															<c:forEach var="ed" items="${resume.education}">
																<div class="resume-item">
																	...
																	<div class="resume-item-line">
																		<span class="resume-item-label-inline">학위 :</span> <span
																			class="resume-item-value-inline"><c:out
																				value="${ed.degree}" /></span>
																	</div>

																	<c:if test="${not empty ed.school}">
																		<div class="resume-item-line">
																			<span class="resume-item-label-inline">학교명 :</span> <span
																				class="resume-item-value-inline"><c:out
																					value="${ed.school}" /></span>
																		</div>
																	</c:if>

																	<c:if test="${not empty ed.major}">
																		<div class="resume-item-line">
																			<span class="resume-item-label-inline">전공 :</span> <span
																				class="resume-item-value-inline"><c:out
																					value="${ed.major}" /></span>
																		</div>
																	</c:if>
																	...
																	<c:if
																		test="${not empty ed.startDate or not empty ed.endDate}">
																		<div class="resume-item-line">
																			<span class="resume-item-label-inline">기간 :</span> <span
																				class="resume-item-value-inline"> <c:if
																					test="${not empty ed.startDate}">
																					<c:out value="${ed.startDate}" />
																				</c:if> <c:if test="${not empty ed.endDate}">
														...
														<c:when test="${not empty resume.certificationItems}">
															<c:forEach var="c" items="${resume.certificationItems}">
																<div class="resume-item">
																	...
																	<div class="resume-item-line">
																		<span class="resume-item-label-inline">자격증명 :</span> <span
																			class="resume-item-value-inline"><c:out
																				value="${c.certi_name}" /></span>
																	</div>

																	<c:if test="${not empty c.organizer}">
																		<div class="resume-item-line">
																			<span class="resume-item-label-inline">주최기관 :</span>
																			<span class="resume-item-value-inline"><c:out
																					value="${c.organizer}" /></span>
																		</div>
																	</c:if>
									...
									<div class="resume-block">
										<h3 class="resume-section-title">Contests</h3>

										<div class="view-mode">
											<c:choose>
												<c:when test="${not empty resume.contestItems}">
													<c:forEach var="ct" items="${resume.contestItems}">
														<div class="resume-item">

															<div class="resume-item-line">
																<span class="resume-item-label-inline">공모전명 :</span>
																<span class="resume-item-value-inline"><c:out value="${ct.cont_name}" /></span>
															</div>

															<c:if test="${not empty ct.organizer}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">주최기관 :</span>
																	<span class="resume-item-value-inline"><c:out value="${ct.organizer}" /></span>
																</div>
															</c:if>

															<c:if test="${not empty ct.deadline}">
																<div class="resume-item-line">
																	<span class="resume-item-label-inline">마감일 :</span>
																	<span class="resume-item-value-inline"><c:out value="${ct.deadline}" /></span>
																</div>
															</c:if>
												...
												<c:otherwise>
													<div class="resume-empty">등록된 공모전이 없습니다.</div>
												</c:otherwise>
											</c:choose>
										</div>

										<div class="edit-mode" id="contests-editor" style="display: none;">
											<c:choose>
												<c:when test="${not empty resume.contestItems}">
													<c:forEach var="ct" items="${resume.contestItems}">
														<div class="edit-section-box contest-row">
															<input type="text" class="form-control mb-2 contest-name"
																value="${fn:escapeXml(ct.cont_name)}" placeholder="공모전명" />
															<input type="text" class="form-control mb-2 contest-org"
																value="${fn:escapeXml(ct.organizer)}" placeholder="주최기관" />
															<input type="text" class="form-control mb-2 contest-deadline"
																value="${fn:escapeXml(ct.deadline)}" placeholder="마감일" />
															<button type="button"
																class="btn btn-sm btn-outline-danger editor-remove-btn"
																onclick="removeEditorRow(this)">삭제</button>
														</div>
												...
												<c:otherwise>
													<div class="edit-section-box contest-row">
														<input type="text" class="form-control mb-2 contest-name" placeholder="공모전명" />
														<input type="text" class="form-control mb-2 contest-org" placeholder="주최기관" />
														<input type="text" class="form-control mb-2 contest-deadline" placeholder="마감일" />
														<button type="button"
															class="btn btn-sm btn-outline-danger editor-remove-btn"
															onclick="removeEditorRow(this)">삭제</button>
													</div>
											...
											<button type="button"
												class="btn btn-sm btn-outline-secondary editor-add-btn"
												onclick="addContestRow()">공모전 추가</button>
										</div>
									</div>
...
<script>
	...
	function removeEditorRow(btn) {
		const row = btn.closest('.link-row, .project-row, .experience-row, .education-row, .cert-row, .contest-row');
		if (row)
			row.remove();
	}
	...
	function addContestRow() {
		const html = ''
				...
				+ '<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>'
				+ '</div>';
		document.getElementById('contests-editor').insertAdjacentHTML(
				'beforeend', html);
	}
	...
	function collectResumeJson() {
		...
		const contestsEditor = document.getElementById('contests-editor');
		if (contestsEditor) {
			contestsEditor.querySelectorAll('.contest-row').forEach(function(row) {
				const cont_name = row.querySelector('.contest-name')?.value?.trim()
						|| '';
				const organizer = row.querySelector('.contest-org')?.value?.trim()
						|| '';
				const deadline = row.querySelector('.contest-deadline')?.value?.trim()
						|| '';
				if (cont_name || organizer || deadline) {
					contestItems.push({
						cont_name : cont_name,
						organizer : organizer,
						deadline : deadline
	...
	function restoreResumeJson(data) {
		...
		const contestsEditor = document.getElementById('contests-editor');
		if (contestsEditor) {
			contestsEditor.innerHTML = '';
			const items = (data.contestItems && data.contestItems.length) ? data.contestItems
					: [ {} ];
			items.forEach(function(item) {
				const html = ''
						+ '<div class="edit-section-box contest-row">'
						+ '<input type="text" class="form-control mb-2 contest-name" value="'
						+ escapeHtml(item.cont_name || '')
						+ '" placeholder="공모전명" />'
						+ '<input type="text" class="form-control mb-2 contest-org" value="'
						+ escapeHtml(item.organizer || '')
						+ '" placeholder="주최기관" />'
						+ '<input type="text" class="form-control mb-2 contest-deadline" value="'
						+ escapeHtml(item.deadline || '')
						+ '" placeholder="마감일" />'
						+ '<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>'
						+ '</div>';
				contestsEditor.insertAdjacentHTML('beforeend', html);
			});
			contestsEditor.insertAdjacentHTML('beforeend',
					'<button type="button" class="btn btn-sm btn-outline-secondary editor-add-btn" onclick="addContestRow()">공모전 추가</button>');
		}
	}
