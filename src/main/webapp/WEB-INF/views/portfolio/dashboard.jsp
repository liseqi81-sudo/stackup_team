									...
									~ <c:out value="${ed.endDate}" />
										...
										<div class="resume-bottom-col">
											<div class="resume-block">
														...
														<c:when test="${not empty resume.contestItems}">
															<c:forEach var="ct" items="${resume.contestItems}">
																<div class="resume-item">
																	...
																	<div class="resume-item-line">
																		<span class="resume-item-label-inline">공모전명 :</span> <span
																			class="resume-item-value-inline"> <c:out
																				value="${ct.contName}" />
																		</span>
																	</div>
																	...
																	<c:if test="${not empty ct.deadline}">
																		<div class="resume-item-line">
																			<span class="resume-item-label-inline">마감일 :</span> <span
																				class="resume-item-value-inline"> <c:out
																					value="${ct.deadline}" />
																			</span>
																		</div>
												...
												<div class="edit-mode" id="contests-editor"
													style="display: none;">
													<c:choose>
														<c:when test="${not empty resume.contestItems}">
															<c:forEach var="ct" items="${resume.contestItems}">
																<div class="edit-section-box contest-row">
																	<input type="text"
																		class="form-control mb-2 contest-name"
																		value="${fn:escapeXml(ct.contName)}"
																		placeholder="공모전명" /> <input type="text"
																		class="form-control mb-2 contest-org"
																		value="${fn:escapeXml(ct.organizer)}"
																		placeholder="주최기관" /> <input type="text"
																		class="form-control mb-2 contest-deadline"
										...
										<div class="resume-block">
												...
												<div class="view-mode">
													<c:choose>
														<c:when test="${not empty resume.contestsItems}">
															<c:forEach var="c" items="${resume.contestsItems}">
																<div class="resume-item">

																	<div class="resume-item-line">
																		<span class="resume-item-label-inline">공모전명 :</span> <span
																			class="resume-item-value-inline"><c:out
																				value="${c.contest_name}" /></span>
																	</div>
														...
														<c:otherwise>
															<div class="resume-empty">등록된 공모전ㅇ 없습니다.</div>
														</c:otherwise>
													</c:choose>
												</div>
												<div class="edit-mode" id="contests-editor"
													...
													<c:choose>
														<c:when test="${not empty resume.contestItems}">
															<c:forEach var="c" items="${resume.contestsItems}">
																<div class="edit-section-box cert-row">
																	<input type="text" class="form-control mb-2 cert-name"
																		value="${fn:escapeXml(c.contest_name)}"
																		placeholder="공모전명" /> <input type="text"
																		class="form-control mb-2 cert-org"
																		value="${fn:escapeXml(c.organizer)}"
																		placeholder="주최기관" />
							...
							<div class="portfolio-card-top">
								...
								<div class="portfolio-meta">
									...
									<form method="post"
										...
										<button type="button" class="btn btn-sm btn-success"
											onclick="saveResumeSnapshot(${p.portfolioId})">현재
											정보로 새 버전 저장</button>
									</form>
								</div>
								...
								<div style="margin-top: 12px;">
									<form method="post"
										...
										style="margin: 0;">
										<button type="button" class="btn btn-sm btn-success"
											onclick="saveResumeSnapshot(${p.portfolioId})">정보
											업데이트</button>
									</form>
								</div>
							</div>
...
<script>
	...
	function addContestRow() {
		const html = ''
				...
				+ '</div>';
		document.getElementById('contests-editor').insertAdjacentHTML('beforeend', html);
	}
	...
	function restoreResumeJson(data) {
		...
		if (contestsEditor) {
			...
			items.forEach(function(item) {
				const html = ''
						+ '<div class="edit-section-box contest-row">'
						+ '<input type="text" class="form-control mb-2 contest-name" value="' + escapeHtml(item.contName || '') + '" placeholder="공모전명" />'
						+ '<input type="text" class="form-control mb-2 contest-org" value="' + escapeHtml(item.organizer || '') + '" placeholder="주최기관" />'
						+ '<input type="text" class="form-control mb-2 contest-deadline" value="' + escapeHtml(item.deadline || '') + '" placeholder="마감일" />'
						+ '<button type="button" class="btn btn-sm btn-outline-danger editor-remove-btn" onclick="removeEditorRow(this)">삭제</button>'
						+ '</div>';
				contestsEditor.insertAdjacentHTML('beforeend', html);
			});

			contestsEditor.insertAdjacentHTML(
				'beforeend',
